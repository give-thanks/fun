import { Guess, GuessType } from "../state/types.js";
...
export function validatePartition(
    ...
    guesses.forEach(g => {
        ...
        const countsOfElementsInGuess = partition.map(group =>
            group.filter(t => g.tileIds.includes(t.id)).length
        );

        // console.log(`${JSON.stringify(partition)} ${countsOfElementsInGuess}`)

        if (g.result === GuessType.Cold) {
            invalid ||= countsOfElementsInGuess.some(count => count >= (partition.length == 2 ? 3 : 4));
        }
        ...
        if (g.result === GuessType.Close) {
            ...
            invalid ||= !countsOfElementsInGuess.some(count => count == 3) || countsOfElementsInGuess.some(count => count == 4);
