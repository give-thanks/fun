...
function getActiveGuessPanels() {
    const panels = new Map();
    [0, 1, 2, 3].forEach(cat => {
        const tilesWithCat = appState.remainingTiles.filter(t => t.categories.has(cat));
        if (tilesWithCat.length === 4)
            panels.set(cat, tilesWithCat);
    });
    return panels;
}
function renderGuessPanels() {
    ...
    activePanels.forEach((tilesForGuess, catId) => {
        ...
        ["correct", "oneAway", "cold"].forEach(result => {
            ...
            btn.addEventListener("click", () => applyCategoryGuess(catId, tilesForGuess, result));
            panel.appendChild(btn);
        });
        guessPanelContainer.appendChild(panel);
    });
}
function applyCategoryGuess(catId, tiles, result) {
    const guess = { id: nextGuessId(), tileIds: tiles.map(t => t.id), result };
